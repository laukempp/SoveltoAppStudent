  ...
  const showResult = () => {
    return studentAnswer.map((item, index) => {
      ...
      if (item.count === 1 && item.isCorrect === true) {
        colorsin = { backgroundColor: "#33dd22" };
      } else if (item.isCorrect === true && item.count === 0) {
        colorsin = { backgroundColor: "#90EE90" };
      } else if (item.isCorrect === false && item.count === 1) {
        colorsin = { backgroundColor: "#eedd9d" };
      }

      return (
        <div key={index} className="text-left" style={colorsin}>
