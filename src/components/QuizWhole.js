...
const QuizWhole = ({formProps}) => {
  ...
  const buttonClassChanger = () => {
    ...
    return (
        ...
        <div className="text-white">Hei {sessionStorage.getItem("nickname")}! Tervetuloa tekemään tenttiä.</div>
        <form>
          <div className="qnbox">
            {questions &&
              questions.length > 0 &&
              questions.map((result, index) => {
                return (
                  <Question index={index} result={result} key={result.id} setMessage={setMessage}/>
                );
              })}
          </div>
